...
def test_hdfs_roots(er_user): assert allowed_hdfs_roots(er_user,"ER")==["/naplatform/departments/ER","/naplatform/users/alice"]
def test_hdfs_denies_other_department(er_user):
    with pytest.raises(AccessDenied): assert_hdfs_path_allowed(er_user,"/naplatform/departments/IT/a","ER")
